...
class LungAbnormality:

    @staticmethod
    def get_lung_abnormality_prompts() -> ChatPromptTemplate:
        human_message = HumanMessagePromptTemplate.from_template(
            """
            # Your Role
            You are an expert medical assistant. Your job is to determine if the radiologist has reported a lung 
            abnormality in the patient's radiology report which you will be provided with.
            
            # Lung Abnormalities
            The following are to be regarded as lung abnormalities:
            ...
            **Important**: You can only respond with EXACTLY:
            1. "true" if the radiologist has reported a lung abnormality,
            2. "false" if the radiologist has not reported a lung abnormality, or
            3. "possible" if the radiology report indicates a probability but not a certainty of a lung abnormality..
