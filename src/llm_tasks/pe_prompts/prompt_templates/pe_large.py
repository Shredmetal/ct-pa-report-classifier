...
class PulmonaryEmbolusLarge:

    @staticmethod
    def get_pulmonary_embolus_large_prompts() -> ChatPromptTemplate:

        human_message = HumanMessagePromptTemplate.from_template(
            """
            # Your Role
            You are an expert medical assistant. Your job is to determine if the radiologist has reported whether the 
            pulmonary embolus or pulmonary embolism is large or not, in the patient's radiology report which 
            ...
            **Important**: You can only respond with EXACTLY:
            1. "true" if the radiologist has reported that pulmonary embolus or pulmonary embolism is large,
            2. "false" if the radiologist has reported that pulmonary embolus or pulmonary embolism is **not** large,
